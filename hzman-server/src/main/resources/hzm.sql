...
drop table hzm.hzm_aws_user;
CREATE TABLE hzm.hzm_aws_user
(
...
(
    `id`             int(11) unsigned NOT NULL AUTO_INCREMENT,
    `user_market_id` int(11) unsigned NOT NULL COMMENT '亚马逊市场账号id',
    `spider_type`    tinyint(4) unsigned NOT NULL COMMENT '爬取类型',
    `spider_depend`  varchar(255) NOT NULL COMMENT '爬去依赖',
    `is_active`      tinyint(4) unsigned NOT NULL COMMENT '是否开启爬取',
    `ctime`          datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `utime`          datetime     NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE `hzm_father_child_relation`
(
    `id`             int(11) unsigned NOT NULL AUTO_INCREMENT,
    `user_market_id` int(11) unsigned NOT NULL COMMENT '亚马逊市场账号id',
    `father_sku`     varchar(255) NOT NULL COMMENT '父sku',
    `father_asin`    varchar(255) NOT NULL COMMENT '父asin',
    `child_sku`      varchar(255) NOT NULL COMMENT '子sku',
    `child_asin`     varchar(255) NOT NULL COMMENT '子asin',
    `ctime`          datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `utime`          datetime     NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY              `idx_father_sku` (`father_sku`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='父子sku关系表'

drop table hzm.hzm_item_inventory;
CREATE TABLE hzm.hzm_item_inventory
(
    `id`                         int(11) unsigned NOT NULL AUTO_INCREMENT,
    `user_market_id`             int(11) unsigned NOT NULL COMMENT '亚马逊市场账号id',
    `asin`                       varchar(128) not null comment 'ASIN',
    ...
    `amazon_quantity`            int(11) unsigned NOT NULL COMMENT '亚马逊可履约数量',
    `fulfillable_quantity`       int(11) unsigned NOT NULL COMMENT '可履约数量',
    `inbound_working_quantity`   int(11) unsigned NOT NULL COMMENT '入库数量',
    `inbound_shipped_quantity`   int(11) unsigned NOT NULL COMMENT '入库数量',
    `inbound_receiving_quantity` int(11) unsigned NOT NULL COMMENT '转移数量',
    `reserved_quantity`          varchar(4096)         default null comment '预留数量',
    `researching_quantity`       varchar(8192)         default null comment '调查数量',
    `unfulfillable_quantity`     varchar(8192)         default null comment '不可履约数量',
    `last_updated_time`          varchar(128) not null comment '最后更新时间',
...
drop table hzm.hzm_amazon_order_finance;
CREATE TABLE hzm.hzm_amazon_order_finance
(
    `id`                  int(11) unsigned NOT NULL AUTO_INCREMENT,
    `amazon_order_id`     varchar(255) NOT NULL COMMENT '亚马逊自定义订单编码',
    `shipment_event_list` mediumtext COMMENT '货物事件',
    `other_event_list`    mediumtext COMMENT '其他事件',
    `ctime`               datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `utime`               datetime     NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY                   `idx_amazon_order_id` (`amazon_order_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='订单财务信息表';
